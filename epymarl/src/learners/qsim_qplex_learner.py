...
from modules.mixers.qsim_dmaq_general import DMAQer
...
class QsimQplexLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Get the Q-values for the maximizing actions from the live network
        mac_out_detach_qplex = mac_out.clone().detach()
        mac_out_detach_qplex[avail_actions == 0] = -9999999
        max_action_qvals, max_action_index = mac_out_detach_qplex[:, :-1].max(dim=3)

        # --- Mixer section for chosen actions (QPLEX logic) ---
        if self.mixer is not None:
            v_tot_chosen = self.mixer(chosen_action_qvals, batch["state"][:, :-1], is_v=True)
            adv_tot_chosen = self.mixer(chosen_action_qvals, batch["state"][:, :-1], actions=onehot_action[:, :-1],
                                      max_q_i=max_action_qvals, is_v=False)
            chosen_action_qvals = v_tot_chosen + adv_tot_chosen

        # --- Target Calculation section (QSIM-QPLEX logic) ---
        with th.no_grad():
            
            mac_out_detach_qplex_next = mac_out.clone().detach()
            mac_out_detach_qplex_next[avail_actions == 0] = -9999999
            cur_max_actions_for_target = mac_out_detach_qplex_next[:, 1:].max(dim=3, keepdim=True)[1]
            target_chosen_qvals = th.gather(target_mac_out, 3, cur_max_actions_for_target).squeeze(3)
            target_max_qvals_qplex = target_mac_out.max(dim=3)[0]
            cur_max_actions_onehot = th.zeros(cur_max_actions_for_target.squeeze(3).shape + (self.args.n_actions,), device=cur_max_actions_for_target.device)
            cur_max_actions_onehot.scatter_(3, cur_max_actions_for_target, 1)

            if self.target_mixer is not None:
                b, t, n_scenarios, a, _ = target_max_qvals.shape
                target_max_qvals_reshaped = target_max_qvals.squeeze(-1).reshape(-1, a)
                state_t_plus_1 = batch["state"][:, 1:]
                state_repeated_for_v = state_t_plus_1.unsqueeze(2).expand(-1, -1, n_scenarios, -1).reshape(-1, self.state_dim)
                target_v_tots_all_scenarios = self.target_mixer(target_max_qvals_reshaped, state_repeated_for_v, is_v=True)
                target_v_tots = target_v_tots_all_scenarios.view(b, t, n_scenarios)
                target_adv_tot_global = self.target_mixer(target_chosen_qvals, batch["state"][:, 1:],
                                                   actions=cur_max_actions_onehot,
                                                   max_q_i=target_max_qvals_qplex, is_v=False)
                target_max_qvals = target_v_tots + target_adv_tot_global
        ...
        if self.args.target_update_interval_or_tau <= 1.0:
            # Soft update
            self._update_targets_soft(self.args.target_update_interval_or_tau)
        elif (self.training_steps - self.last_target_update_step) / self.args.target_update_interval_or_tau >= 1.0:
            # Hard update
    ...
    def _update_targets_hard(self):
        ...
        self.logger.console_logger.info("Updated target network (hard update)")
