...
class MAXQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # 检查是否到了更新目标网络的时机
        if self.args.target_update_interval <= 1.0:
            # Soft update
            self._update_targets_soft(self.args.target_update_interval)
        elif (self.training_steps - self.last_target_update_step) / self.args.target_update_interval >= 1.0:
            # Hard update
    ...
    def _update_targets_soft(self, tau):
        ...
        if self.central_mac is not None:
            for target_param, param in zip(self.target_central_mac.parameters(), self.central_mac.parameters()):
                target_param.data.copy_(target_param.data * (1.0 - tau) + param.data * tau)
                
        if self.central_mixer is not None:
            for target_param, param in zip(self.target_central_mixer.parameters(), self.central_mixer.parameters()):
                target_param.data.copy_(target_param.data * (1.0 - tau) + param.data * tau)
